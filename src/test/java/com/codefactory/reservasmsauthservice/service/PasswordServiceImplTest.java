...
class PasswordServiceImplTest {
    ...
    void setUp() {
        ...
        when(userRepository.findClientNameByUserId(any())).thenReturn(Optional.of("Carlos Pérez"));
    ...
    @Test
    @DisplayName("requestPasswordReset: email registrado → genera token y envía correo")
    void requestPasswordReset_EmailRegistrado_GeneraTokenYEnviaCorreo() {
        ...
        verify(emailService, times(1)).sendPasswordResetEmail(eq("carlos@email.com"), anyString(), anyString());
    }

    @Test
    @DisplayName("requestPasswordReset: email no registrado → no falla (seguridad) y no envía correo")
    ...
    @Test
    @DisplayName("confirmPasswordReset: token válido y contraseña válida → restablece contraseña")
    void confirmPasswordReset_TokenValidoContrasenaValida_RestablaceContrasena() {
        ...
        // Act
        MessageResponseDTO resultado = passwordService.confirmPasswordReset(request);
    ...
    @Test
    @DisplayName("confirmPasswordReset: token inválido → lanza InvalidResetTokenException")
    ...
    @Test
    @DisplayName("confirmPasswordReset: contraseña sin mayúscula → lanza InvalidPasswordException")
    ...
    @Test
    @DisplayName("confirmPasswordReset: contraseña menor a 8 caracteres → lanza InvalidPasswordException")
    ...
    @Test
    @DisplayName("changePassword: usuario autenticado con contraseña correcta → cambia contraseña")
    void changePassword_UsuarioAutenticadoContrasenaCorrecta_CambiaContrasena() {
        // Arrange
        SecurityContextHolder.getContext().setAuthentication(
                new UsernamePasswordAuthenticationToken("carlos@email.com", null, List.of()));
        ...
        when(userRepository.findByEmail("carlos@email.com")).thenReturn(Optional.of(usuario));
        when(passwordEncoder.matches("Segura#123", "$2a$10$hashActual")).thenReturn(true);
        when(passwordEncoder.matches("NuevaSegura#1", "$2a$10$hashActual")).thenReturn(false);
        when(passwordEncoder.encode("NuevaSegura#1")).thenReturn("$2a$10$nuevoHash");
        when(userRepository.save(usuario)).thenReturn(usuario);
    ...
    @Test
    @DisplayName("changePassword: contraseña actual incorrecta → lanza IncorrectPasswordException")
    void changePassword_ContrasenaActualIncorrecta_LanzaIncorrectPasswordException() {
        ...
        when(userRepository.findByEmail("carlos@email.com")).thenReturn(Optional.of(usuario));
        when(passwordEncoder.matches("ContrasenaMal#1", "$2a$10$hashActual")).thenReturn(false);
    ...
    @Test
    @DisplayName("changePassword: nueva contraseña igual a la actual → lanza SamePasswordException")
    ...
    @Test
    @DisplayName("changePassword: sin autenticación → lanza AuthenticationCredentialsNotFoundException")
