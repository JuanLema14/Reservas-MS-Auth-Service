...
class ProviderServiceImplTest {
    ...
    @Test
    @DisplayName("CP-02-001: Crear proveedor con datos válidos → retorna ProviderResponseDTO")
    void createProvider_DatosValidos_RetornaProviderResponseDTO() {
        ...
        when(catalogClient.getCategoryById(categoriaId)).thenReturn(categoriaActiva);
    ...
    @Test
    @DisplayName("CP-02-001b: Crear proveedor → contraseña hasheada antes de guardar")
    ...
    @Test
    @DisplayName("CP-02-002: Categoría no encontrada (FeignException) → lanza CategoryNotFoundException")
    ...
    @Test
    @DisplayName("CP-02-002b: Categoría inactiva → lanza CategoryNotFoundException")
    void createProvider_CategoriaInactiva_LanzaCategoryNotFoundException() {
        ...
        doNothing().when(userAuthService).validateEmailAndPassword(any(), any());
        when(catalogClient.getCategoryById(categoriaId)).thenReturn(categoriaInactiva);
    ...
    @Test
    @DisplayName("CP-02-005: Correo ya en uso → lanza EmailAlreadyExistsException sin llegar a guardar")
    void createProvider_CorreoDuplicado_LanzaExcepcionSinGuardar() {
        ...
        doThrow(new EmailAlreadyExistsException("El correo electrónico ya está en uso"))
                .when(userAuthService).validateEmailAndPassword("salon@bellavida.com", "Segura#123");
    ...
    @Test
    @DisplayName("getUserEntityByEmail: email existente → retorna entidad Provider")
    void getUserEntityByEmail_EmailExistente_RetornaProvider() {
        ...
        // Act
        User resultado = providerService.getUserEntityByEmail("salon@bellavida.com");
    ...
    void getUserEntityByEmail_EmailNoExistente_LanzaResourceNotFoundException() {
        // Arrange
        when(providerRepository.findByEmail("noexiste@email.com")).thenReturn(Optional.empty());

        // Act & Assert
        assertThatThrownBy(() -> providerService.getUserEntityByEmail("noexiste@email.com"))
    ...
    @Test
    @DisplayName("getExternalProviderById: ID existente → retorna ExternalProviderDTO")
    void getExternalProviderById_IdExistente_RetornaExternalProviderDTO() {
        ...
        when(providerRepository.findById(proveedorId)).thenReturn(Optional.of(proveedorEntity));
    ...
    @Test
    @DisplayName("getExternalProviderById: ID no existente → lanza ResourceNotFoundException")
    void getExternalProviderById_IdNoExistente_LanzaResourceNotFoundException() {
        // Arrange
        UUID idDesconocido = UUID.randomUUID();
        when(providerRepository.findById(idDesconocido)).thenReturn(Optional.empty());

        // Act & Assert
        assertThatThrownBy(() -> providerService.getExternalProviderById(idDesconocido))
                .isInstanceOf(ResourceNotFoundException.class)
                .hasMessageContaining("no encontrado");
    }
}
